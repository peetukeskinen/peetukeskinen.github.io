...
Three reusable patterns are documented as HTML comments in the files themselves:
...
- **Timeline entry** — see the template comment in `cv.html`. A `<li>` holds a `.when`
  date for the left gutter and a `.what` block for the position, institution and any
  status marker.

### Numbering

Publication numbers and section numbers are both generated by CSS counters, never
typed into the HTML. Add, reorder or delete an entry or a `<section>` and everything
renumbers itself. Publications restart at 1 in each section.
